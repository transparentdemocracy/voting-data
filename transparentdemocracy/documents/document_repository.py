...
# TODO: split into 'LocalDocumentRepository' and 'GoogleDriveDocumentRepository' to have less mixed concerns
class DocumentGoogleDriveRepository:
    ...
    def __init__(self, config, local_document_text_dir):
        self.config = config
        ...
        self.summary_dir_id = None
        self.remote_storage_root = self.find_directory_by_name('storage')
    ...
    def existing_remote_summary_document(self, document_id):
        existing_files = self.service.files().list(q=f"name='{document_id}' and parents in '{self.summary_dir_id}'").execute()['files']
        file_id = existing_files[0]['id'] if existing_files else None
        return file_id

    def _replace_remote_file(self, file_id, local_path, mimetype="text/plain"):
    ...
    def document_id_to_text_path(self, document_id):
        return os.path.join(self.local_document_text_dir, self.config.leg_dir, document_id[3:5], document_id[5:7], f"{document_id}.txt")

    def document_id_to_summary_path(self, document_id):
        return os.path.join(self.local_document_text_dir, self.config.leg_dir, document_id[3:5], document_id[5:7], f"{document_id}.summary")

    def ensure_text_dir_exists(self):
        if self.text_dir_id is None:
            self.text_dir_id = self.ensure_directories_exist(self.config.google_drive_text_dir.split('/'))

    def ensure_summary_dir_exists(self):
        if self.summary_dir_id is None:
            self.summary_dir_id = self.ensure_directories_exist([self.remote_storage_root, 'documents', 'summary', f'leg-{self.legislature}'])
    ...
    def upsert_document_summary(self, document_id):
        self.ensure_summary_dir_exists()

        local_path = self.document_id_to_summary_path(document_id)
        if not os.path.exists(local_path):
            raise Exception(f"Can't find summary file for document id {document_id} at {local_path}")

        file_id = self.existing_remote_summary_document(document_id)
        if file_id:
            self._replace_remote_file(file_id, local_path, )
        else:
            self._upload_summary_file(local_path, document_id, self.summary_dir_id)
