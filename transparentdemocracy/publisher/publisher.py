...
def publish():
    ...
    # Initialize a defaultdict to hold our grouped objects
    ...
    for plenary in plenaries:
        for mg in plenary["motion_groups"]:
            motions = [to_motion_read_model(plenary, mg, m, votes_by_id) for m in mg["motions"]]
...
def to_motion_read_model(p, mg, m, votes_by_id):
    ...
    yes_votes = to_votes(votes, "YES")
    mdoc["yesVotes"] = yes_votes
    no_votes = to_votes(votes, "NO")
    mdoc["noVotes"] = no_votes
    mdoc["absVotes"] = to_votes(votes, "ABSTENTION")
