...
class ReportItemExtractionTest(unittest.TestCase):
    ...
    def assert_report_item(self, report_item: ReportItem,
        ...
        self.assertEqual(expected_nl_title_prefix,
                         report_item.nl_title[:len(expected_nl_title_prefix)])
        self.assertEqual(expected_fr_title_prefix,
...
class MotionExtractionTest(unittest.TestCase):
    ...
    def test_extract_motions__ip298x_html__go_to_example_report(self):
        ...
        self.assertTrue(motion_group10.title_nl.startswith(
            "Moties ingediend tot besluit van de interpellaties van - Koen Metsu over "))
        self.assertTrue(motion_group10.title_nl.endswith(
            'Comité P over de aanslag van 16 oktober 2023" (nr. 486)'))
        self.assertTrue(motion_group10.title_fr.startswith(
            "Motions déposées en conclusion des interpellations de - Koen Metsu sur"))
        self.assertTrue(motion_group10.title_fr.endswith(
            ' sur l\'attentat du 16 octobre 2023" (n° 486)'))
        ...
        self.assertEqual("10", motion_group10.motions[0].sequence_number)
        self.assertTrue(motion_group10.motions[0].title_nl.startswith(
            "Moties ingediend tot besluit van de interpellaties van - Koen Metsu over "))
        self.assertTrue(motion_group10.motions[0].title_nl.endswith(
            'Comité P over de aanslag van 16 oktober 2023" (nr. 486)'))
        self.assertTrue(motion_group10.motions[0].title_fr.startswith(
            "Motions déposées en conclusion des interpellations de - Koen Metsu sur"))
        self.assertTrue(motion_group10.motions[0].title_fr.endswith(
            ' sur l\'attentat du 16 octobre 2023" (n° 486)'))
        ...
        self.assertTrue(motion_group10.motions[0].description.endswith(
        ...
        self.assertEqual("14", motion_group14.motions[0].sequence_number)
    ...
    def test_extract_motions__ip262x_html__go_to_example_report(self):
        ...
        self.assertEqual(Motion("55_262_mg_12_m0", "12",
...
class VoteExtractionTest(unittest.TestCase):
    ...
    def test_extract_votes_ip298x__go_to_example_report(self):
        ...
        self.assertEqual("Aouasti Khalil",
                         voting1_votes_yes[0].politician.full_name)
        self.assertEqual("Bacquelaine Daniel",
                         voting1_votes_yes[1].politician.full_name)
        self.assertEqual(
            "Wilmès Sophie", voting1_votes_yes[-2].politician.full_name)
        self.assertEqual("Zanchetta Laurence",
                         voting1_votes_yes[-1].politician.full_name)

        voting1_votes_no = [
            vote for vote in voting1_votes if vote.vote_type is VoteType.NO]
        self.assertEqual(50, len(voting1_votes_no))
        self.assertEqual(
            "Anseeuw Björn", voting1_votes_no[0].politician.full_name)
        self.assertEqual(
            "Bruyère Robin", voting1_votes_no[1].politician.full_name)
        self.assertEqual(
            "Verreyt Hans", voting1_votes_no[-2].politician.full_name)
        self.assertEqual(
            "Wollants Bert", voting1_votes_no[-1].politician.full_name)

        voting1_votes_abstention = [
            vote for vote in voting1_votes if vote.vote_type is VoteType.ABSTENTION]
        self.assertEqual(4, len(voting1_votes_abstention))
        self.assertEqual(
            "Arens Josy", voting1_votes_abstention[0].politician.full_name)
        self.assertEqual(
            "Daems Greet", voting1_votes_abstention[1].politician.full_name)
        self.assertEqual("Rohonyi Sophie",
                         voting1_votes_abstention[-2].politician.full_name)
        self.assertEqual("Vindevoghel Maria",
                         voting1_votes_abstention[-1].politician.full_name)

        self.assertEqual(133, len(voting1_votes))

        self.assertEqual(
            134, len([v for v in votes if v.voting_id == "55_298_v2"]))

        self.assertEqual(
            132, len([v for v in votes if v.voting_id == "55_298_v3"]))
...
class PlenaryExtractionTest(unittest.TestCase):
    ...
    def test_extract_from_html_plenary_report__ip298x_html__go_to_example_report(self):
        ...
        self.assertEqual(
            "https://www.dekamer.be/doc/PCRI/pdf/55/ip298.pdf", plenary.pdf_report_url)
        self.assertEqual(
            "https://www.dekamer.be/doc/PCRI/html/55/ip298x.html", plenary.html_report_url)
        ...
        self.assertEqual(
            ['Aouasti Khalil', 'Bacquelaine Daniel'], yes_voters[:2])
    ...
    def test_extract_from_html_plenary_report__ip261x_html__different_proposals_header(self):
        ...
        self.assertEqual(
            "https://www.dekamer.be/doc/PCRI/pdf/55/ip261.pdf", plenary.pdf_report_url)
        self.assertEqual(
            "https://www.dekamer.be/doc/PCRI/html/55/ip261x.html", plenary.html_report_url)
        ...
        self.assertEqual("Demande d'avis du Conseil d'État",
                         plenary.proposal_discussions[0].proposals[0].title_fr)
        self.assertIsNone(
            plenary.proposal_discussions[0].proposals[0].documents_reference)
    ...
    def test_extract_from_html_plenary_report__ip224x_html__different_proposals_header(self):
        ...
        self.assertEqual(55, plenary.legislature)
        self.assertEqual(
            "https://www.dekamer.be/doc/PCRI/pdf/55/ip224.pdf", plenary.pdf_report_url)
        self.assertEqual(plenary.html_report_url,
                         "https://www.dekamer.be/doc/PCRI/html/55/ip224x.html")
        ...
        self.assertEqual(
            "2931/1-6", plenary.proposal_discussions[0].proposals[0].documents_reference)
        ...
        self.assertEqual(
            "2934/1-30", plenary.proposal_discussions[0].proposals[4].documents_reference)
        ...
        self.assertEqual(
            "1678/1-3", plenary.proposal_discussions[3].proposals[0].documents_reference)
    ...
    def test_extract_from_html_plenary_report__ip245x_html(self):
        ...
        self.assertEqual(
            15, plenary.proposal_discussions[0].plenary_agenda_item_number)
        self.assertEqual(
            19, plenary.proposal_discussions[-1].plenary_agenda_item_number)
    ...
    def test_extract_from_html_plenary_report__ip219x_html(self):
        ...
        self.assertEqual(6, len(plenary.proposal_discussions))
        self.assertEqual(
            11, plenary.proposal_discussions[0].plenary_agenda_item_number)
        self.assertEqual(
            16, plenary.proposal_discussions[5].plenary_agenda_item_number)
    ...
    def test_extract_from_html_plenary_report__ip200x_html(self):
        ...
        self.assertEqual(55, plenary.legislature)
        self.assertEqual(
            "https://www.dekamer.be/doc/PCRI/pdf/55/ip200.pdf", plenary.pdf_report_url)
        self.assertEqual(
            "https://www.dekamer.be/doc/PCRI/html/55/ip200x.html", plenary.html_report_url)
    ...
    def test_extract_from_html_plenary_report__ip184x_html(self):
        ...
        self.assertEqual(
            1, plenary.proposal_discussions[0].plenary_agenda_item_number)
        self.assertEqual(
            4, plenary.proposal_discussions[-1].plenary_agenda_item_number)
    ...
    def test_extract_from_html_plenary_report__ip144x_html(self):
        ...
        self.assertEqual(
            14, plenary.proposal_discussions[0].plenary_agenda_item_number)
        self.assertEqual(
            25, plenary.proposal_discussions[-1].plenary_agenda_item_number)
    ...
    def test_extract_from_html_plenary_report__ip125x_html(self):
        ...
        self.assertEqual(2, len(plenary.proposal_discussions))
        self.assertEqual(
            3, plenary.proposal_discussions[0].plenary_agenda_item_number)
        self.assertEqual(
            4, plenary.proposal_discussions[-1].plenary_agenda_item_number)
    ...
    def test_extract_from_html_plenary_report__ip099x_html(self):
        ...
        self.assertEqual(
            13, plenary.proposal_discussions[0].plenary_agenda_item_number)
        self.assertEqual(
            26, plenary.proposal_discussions[-1].plenary_agenda_item_number)
    ...
    def test_extract_from_html_plenary_report__ip038x_html(self):
        ...
        self.assertEqual(
            10, plenary.proposal_discussions[0].plenary_agenda_item_number)
        self.assertEqual(
            18, plenary.proposal_discussions[-1].plenary_agenda_item_number)
    ...
    def test_extract_from_html_plenary_report__ip021x_html(self):
        ...
        self.assertEqual(
            11, plenary.proposal_discussions[0].plenary_agenda_item_number)
        self.assertEqual(
            16, plenary.proposal_discussions[-1].plenary_agenda_item_number)
    ...
    def test_extract_from_html_plenary_report__ip005x_html(self):
        ...
        self.assertEqual(4, len(plenary.proposal_discussions))
        self.assertEqual(
            23, plenary.proposal_discussions[0].plenary_agenda_item_number)
        self.assertEqual(
            26, plenary.proposal_discussions[-1].plenary_agenda_item_number)
