...
@pytest.mark.skipif(os.environ.get("SKIP_SLOW", None) is not None, reason="This test isn't really slow but requires data")
def test_link_motions_with_proposals__all_plenaries__not_throwing():
    # Arrange
    ...
    # Detect if much more problems started to appear than before.

    assert len(link_problems) <= 79
...
def test_link_motions_with_proposals__ip_298x_html__links_to_correct_proposals():
        ...
        assert ["55_296_d26"] == documents_reference_object_3515.proposal_discussion_ids


        plenary296 = [
            plenary for plenary in plenaries if plenary.number == 296][0]
        ...
        assert 1 == len(matching_proposal_discussion26.proposals)
        assert "3515/1-9" == matching_proposal_discussion26.proposals[0].documents_reference

        # Assert: link motions with proposals with the same main document number,
        # even if sub-documents are different:
        assert "3515/10" == motion_group11.motions[0].documents_reference
        assert ["55_296_d26_p0"] == documents_reference_object_3515.proposal_ids

        matching_proposal0 = [proposal for proposal in matching_proposal_discussion26.proposals
                              if proposal.id == "55_296_d26_p0"][0]
        assert "3515/1-9" == matching_proposal0.documents_reference



def test_link_motions_with_proposals__ip_296x_html():
    ...
    assert "3515/1-9" == motion_group46.documents_reference
    ...
    assert "3515/9" == motion_group46.motions[0].documents_reference
...
def test_link_motions_with_proposals__ip_262x_html__links_to_correct_proposals():
    ...
    # Assert
    plenary262 = [
        plenary for plenary in plenaries if plenary.number == 262][0]
    motion_group12 = plenary262.motion_groups[4]
    assert "3495/1-5" == motion_group12.documents_reference
    ...
    assert ["55_261_d22"] == documents_reference_object_3495.proposal_discussion_ids
    assert "3495/5" == motion_group12.motions[0].documents_reference
    assert "3495/5" == motion_group12.motions[1].documents_reference
    assert "3495/5" == motion_group12.motions[2].documents_reference
    assert ["55_261_d22_p0"] == documents_reference_object_3495.proposal_ids

def test_link_motions_with_proposals__ip_226x_html__multiple_proposal_discussions():
    ...
    assert ["55_222_d03", "55_223_d01", "55_224_d01"] == documents_reference_object_2931.proposal_discussion_ids
    assert "2931/7" == motion_group62.motions[0].documents_reference
    assert "2931/7" == motion_group62.motions[1].documents_reference
    assert "2931/7" == motion_group62.motions[2].documents_reference
    assert "2931/7" == motion_group62.motions[3].documents_reference
